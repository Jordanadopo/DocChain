// Initialize Firebase
   var production_config = {
    ...
    var fileEvent, folderEvent, uploadType;
    /*files.addEventListener("drop", function(event){
      ...
      function uploadMain(){
        ...
        document.cookie = "key="+document.getElementById("form1-password").value+"; expires=Thu, 31 Dec 2130 12:00:00 UTC; path=/";
        ...
        switch(uploadType){
          case "drop_folder": {
            ...
            //close the password dailog box
            document.getElementById("close").click();

            uploadFolder();

            //get the password from the user or the cookie
    ...
    data_items.addEventListener("drop", function(event){
      ...
      if(items[0].webkitGetAsEntry().isDirectory){
        //IF THE DROPPED ITEM IS A FOLDER
        uploadType = "drop_folder";
    ...
    function uploadFolder(){
      upload_status_text.innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Téléchargemnt...&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
      ...
      function uploadFolderToServer(files, details){
        ...
        $.ajax({
          ...
          success: function (data) {
              ...
              addFolderToFireBase(firebaseActiveAccount, data.hash, data.path, data, data.size, "clusterlabs.ipfscloud/folder");
    ...
    function intiateFileUpload(){
      ...
      uploadType = "select_file";

      var addFileCard = document.getElementById("addFileCard");
      uploadFiles = addFileCard.files[0];

      console.log("INNER FUNCTION: "+uploadFiles);

      document.getElementById("setPass").click();
    }


    function setPasswordAndUpoadFile(){
      ...
      var perSecondDelay = Math.round(((uploadFiles.size/61781.125)*1000)/100);
    ...
    function exploreFolder(key, rootDir){
      ...
      userDocRef.get().then((doc) => {
        if(doc && doc.exists){
          ...
          var dirStructure = data.private[key].contents;
